...
CREATE TABLE payments (
    ...
    school_id    UUID NOT NULL REFERENCES schools(id_schools) ON DELETE CASCADE,
    student_id   UUID NOT NULL REFERENCES students(id_students_students),
    service_id   UUID NOT NULL REFERENCES services(id_services),
    amount       NUMERIC(12, 2) NOT NULL,
    payment_date DATE NOT NULL,
    method       VARCHAR(50) CHECK (method IN ('Dinheiro','Transferência','Multicaixa','Outro')),
    reference    VARCHAR(100),
    recorded_by  UUID REFERENCES employees(id_employees),--traducao: registrado por (funcionário que processou o pagamento)
    created_at   TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
